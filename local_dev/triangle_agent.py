    ...
    pipeline_config = PipelineAgentConfig(
        agent_configs=[
            ...
            },
            {
                ...
                "messages": [
                    {"role": "system", "content": system_prompmt},
                    {"role": "user", "content": "You are given a two lists of emotions and their descriptions. "
                                                "You need to write both lists. just rewrite the entire text below the text below."
                                                "\n{first} \n\n\n{second}",},
                ]
            },
        ],
        edges=[("first", "second"), ("first", "third"), ("second", "third")],
        input_id="first",
        output_id="third",
    )

    pipeline = Pipeline(pipeline_config, llm_client)
    context = AgentContext(data={"input": "5"})
